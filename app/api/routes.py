...
async def list_tasks(status: str | None = None, db: Database = Depends(_get_db)):
    task_status = TaskStatus(status) if status else None
    tasks = await db.list_tasks(task_status)
...
async def agent_start(agent: AgentWorker = Depends(_get_agent)):
